...
import { TriggerCache } from "./trigger";

// Vendored from @solid-primitives/map (the `next` branch, which targets Solid
// 2.0): https://github.com/solidjs-community/solid-primitives/tree/next/packages/map
// The published package is still pinned to Solid 1.x internals and the 2.0 port
// has not been released to npm yet. The only change from upstream is importing
// `TriggerCache` from the locally vendored `./trigger` instead of the
// (not-yet-ported) `@solid-primitives/trigger` package.
